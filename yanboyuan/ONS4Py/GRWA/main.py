...
def main():
    ...
    if args.evaluate:
        ...
        env = Service.RwaGame(net_config=args.net, wave_num=args.wave_num, rou=args.rou, miu=args.miu,
        ...
        for model_file in reversed(sorted(os.listdir(directory), key=lambda item: int(item.split('.')[0]))):
            ...
            for t in range(times):
                ...
                while not done:
                    ...
                    total_reward += reward
                    if reward == Service.ARRIVAL_OP_OT:
    ...
    for updata_i in range(update_begin, num_updates):
        ...
        for step in range(args.num_steps):
            ...
            allocated_services += (reward == Service.ARRIVAL_OP_OT).sum()  # 计算分配成功的reward的次数
            if args.step_over.startswith('one_time'):
...
def make_env(net_config: str, wave_num: int, rou: float, miu: float,
             ...
             weight, step_over):
    def _thunk():
        rwa_game = Service.RwaGame(net_config=net_config, wave_num=wave_num, rou=rou, miu=miu,
